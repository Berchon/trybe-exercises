...
SELECT id, 6.8, 450000000, 370000000 from Movies WHERE title = 'Procurando Nemo';
...
WHERE id = 3;
-- WHERE (SELECT id FROM Pixar.Movies WHERE title LIKE 'rat%');
...
DELETE FROM Pixar.BoxOffice WHERE movie_id
IN (SELECT id FROM Movies WHERE director = 'Andrew Staton');

DELETE FROM Pixar.Movies WHERE id
IN (2, 9);
